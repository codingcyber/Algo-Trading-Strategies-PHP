...
foreach ($stocks as $stock) {
	...
	$curl = curl_init();
	//https://www.alphavantage.co/query?function=SMA&symbol=$curlsymbol&interval=daily&time_period=20&series_type=close&apikey=$key
    curl_setopt_array($curl, array(
        CURLOPT_URL => "https://www.alphavantage.co/query?function=SMA&symbol=$curlsymbol&interval=daily&time_period=20&series_type=close&apikey=$key",
    ...
    if($err){
        ...
        $dates = array_keys($data['Technical Analysis: SMA']);
        foreach ($dates as $date) {
		    ...
		    if($count == 1){
		    	...
		    	if(isset($data['Technical Analysis: SMA'][$date]) & !empty($data['Technical Analysis: SMA'][$date])){
		                //if($data['Time Series (Daily)'][$date]['1. open'] != '0.0000'){

		                    // Update into stock_daily_values table
		                    $smasql = "UPDATE stock_daily_values SET 20sma=:sma WHERE stockid=:stockid AND trade_date=:trade_date";
		                    $smaresult = $db->prepare($smasql);
		                    $values = array(':stockid'      => $stockid,
		                                    ':sma'      => $data['Technical Analysis: SMA'][$date]['SMA'],
		                                    ':trade_date'   => $date
		                                    );

		                    $smares = $smaresult->execute($values) or die(print_r($smaresult->errorInfo(), true));
		                    echo $date ." - " . $stock['symbol'] . " 20 SMA Added<br>";
