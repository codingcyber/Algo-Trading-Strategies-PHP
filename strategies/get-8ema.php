...
foreach ($stocks as $stock) {
    ...
    curl_setopt_array($curl, array(
        CURLOPT_URL => "https://www.alphavantage.co/query?function=EMA&symbol=$curlsymbol&interval=daily&time_period=8&series_type=close&apikey=$key",
		    ...
		    if($count == 1){
		    	...
		    	if(isset($data['Technical Analysis: EMA'][$date]) & !empty($data['Technical Analysis: EMA'][$date])){
		                //if($data['Time Series (Daily)'][$date]['1. open'] != '0.0000'){
		                    ...
		                    $smasql = "UPDATE stock_daily_values SET 8ema=:ema WHERE stockid=:stockid AND trade_date=:trade_date";
		                    ...
		                    echo $date ." - " . $stock['symbol'] . " 8 EMA Added<br>";
